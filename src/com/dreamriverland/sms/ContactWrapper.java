...
public class ContactWrapper {
	...
	public static final String LOGTAG = ContactWrapper.class.getSimpleName();
	...
	private static final String[] PEOPLE_PROJECTION_ECLAIR = new String[] {
			CONTACT_ID_ECLAIR, CONTACT_DISPLAY_NAME_ECLAIR };
	...
	public static Uri getEmailLookupContentFilterUri() {
		if (SmsPopupUtils.PRE_ECLAIR) {
			return Uri
					.parse("content://contacts/people/with_email_or_im_filter");
		}
	...
	public static String[] getEmailLookupProjection() {
		if (SmsPopupUtils.PRE_ECLAIR) {
			return new String[] { Contacts.People._ID,
	...
	public static InputStream openContactPhotoInputStream(ContentResolver cr,
		...
		if (SmsPopupUtils.PRE_ECLAIR) {
				...
				try {
					if (cursor.moveToFirst()) {
						...
						if (data != null) {
							...
							Log.v(LOGTAG,
									"openContactPhotoInputStream(): contact photo found");
			...
			Log.v(LOGTAG,
					"openContactPhotoInputStream(): looking in Contacts.Photos");
			/*
		...
		if (preparedEclairSDK) {
			try {
				InputStream photoStream = (InputStream) contactsOpenPhotoStreamMethod
						.invoke(contactsClass, cr,
				...
				Log.e(LOGTAG,
						"Unable to fetch contact photo using Anroid 2.0+ SDK: "
								+ e.toString());
			...
			// contacts database
			/*
			 * Log.v("Looking for contact photo in Data table"); cursor =
			 * cr.query(contactsDataContentUri, // new String[]
			 * {ContactsContract.CommonDataKinds.Photo.PHOTO,
			 * ContactsContract.Data.MIMETYPE}, //
			 * ContactsContract.Data.MIMETYPE + "== '" +
			 * ContactsContract.CommonDataKinds.Photo.CONTENT_ITEM_TYPE +
			 * "' AND " + // ContactsContract.Data.CONTACT_ID + " == " + id,
			 * null, null); new String[] {"data15", "mimetype"}, "mimetype" +
			 * "== '" + "vnd.android.cursor.item/photo" + "' AND " +
			 * "contact_id" + " == " + id, null, null); if (cursor == null)
			 * return null;
			 * 
			 * try { Log.v("CURSOR COUNT = " + cursor.getCount()); while
			 * (cursor.moveToNext()) { byte[] data = cursor.getBlob(0); if (data
			 * != null) { if (Log.DEBUG)Log.v(
			 * "openContactPhotoInputStream(): contact photo found using Data table"
			 * ); return new ByteArrayInputStream(data); } else { if (Log.DEBUG)
			 * Log.v("PHOTO DATA WAS NULL"); } } } finally { cursor.close(); }
			 */
		}
	...
	public static Uri getLookupUri(long contactId, String lookupKey) {
		...
		Uri lookupUri = null;
		try {
			lookupUri = (Uri) contactsGetLookupUri.invoke(contactsClass,
					contactId, lookupKey);
		} catch (IllegalArgumentException e) {
			Log.e(LOGTAG,
					"Unable to get contact lookup Uri using Anroid 2.0+ SDK: "
							+ e.toString());
		} catch (IllegalAccessException e) {
			Log.e(LOGTAG,
					"Unable to get contact lookup Uri using Anroid 2.0+ SDK: "
	...
	public static Uri getLookupUri(ContentResolver cr, Uri contactUri) {
		...
		Uri lookupUri = null;
		try {
			lookupUri = (Uri) contactsGetLookupUriByContactUri.invoke(
					contactsClass, cr, contactUri);
		} catch (IllegalArgumentException e) {
			Log.e(LOGTAG,
					"Unable to get contact lookup Uri using Anroid 2.0+ SDK: "
							+ e.toString());
	...
	public static void showQuickContact(Context context, View target,
		...
		try {
			showQuickContactMethod.invoke(quickContactClass, context, target,
					lookupUri, mode, excludeMimes);
		} catch (IllegalArgumentException e) {
			Log.e(LOGTAG,
					"Unable to get show quick contact dialog using Anroid 2.0+ SDK: "
							+ e.toString());
		} catch (IllegalAccessException e) {
			Log.e(LOGTAG,
					"Unable to get show quick contact dialog using Anroid 2.0+ SDK: "
	...
	private static void prepareEclairSDK() {
		try {
			...
			contactsOpenPhotoStreamMethod = contactsClass.getMethod(
			...
			contactsGetLookupUriByContactUri = contactsClass.getMethod(
					"getLookupUri", new Class[] { ContentResolver.class,
							Uri.class });

			// ContactsContract.QuickContacts class and method
			quickContactClass = Class
					.forName("android.provider.ContactsContract$QuickContact");

			showQuickContactMethod = quickContactClass.getMethod(
					"showQuickContact", new Class[] { Context.class,
			...
			Log.e(LOGTAG,
					"Unable to prepare Anroid 2.0+ SDK functions: "
							+ e.toString());
