...
public class CheckService extends WakefulIntentService {
	...
	protected void doWakefulWork(Intent intent) {
		...
		WeiboService relayer = WeiboService.get();
		if (relayer == null) {
	...
	protected void doCheckWork(WeiboService relayer) {
		try {
		...
		try {
			...
			if (!relayer.isConnectionToggled()) {
				try {
					Log.d(TAG,
							"Error marking deliveries on the server, toggling connection",
							t);
					relayer.toggleConnection();
					// relayer.sendPendingMessagesToServer();
				} catch (Throwable tt) {
	...
	public static void schedule(Context context) {
		WakefulIntentService.scheduleAlarms(
				new com.dreamriverland.sms.AlarmListener(), context);
	}
}